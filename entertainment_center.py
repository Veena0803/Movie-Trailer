...
movies = []
request = \
    Request('https://api.themoviedb.org/3/discover/movie?api_key=b9c30174215534f1820b08ea1cc7bce5&language=en-US&sort_by=popularity.desc&include_adult=false&page=1&append_to_response=videos'
            )

# Request to play youtube videos for the movies

trailer_request = \
    Request('https://api.themoviedb.org/3/movie/263115?api_key=b9c30174215534f1820b08ea1cc7bce5&apprend_to_response=videos'
            )
try:
    ...
    # Iterate through the movie results fetched from the API and selecting only the first 6 movies from the response fetched

    for movie in islice(json_object['results'], 12):
        trailer_response = urlopen('https://api.themoviedb.org/3/movie/'
                                    + str(movie['id'])
...
stranger_things = tv_shows.TV('Stranger Things',
...
got = tv_shows.TV('Game of Thrones',
                  'Series anout a deadly game for control of the Seven Kingdoms and sit atop the Iron Throne'
                  ,
                  'https://upload.wikimedia.org/wikipedia/en/9/92/Game_of_Thrones_Season_7.png'
                  , 'https://www.youtube.com/watch?v=giYeaKsXnsI')

office = tv_shows.TV('The Office',
...
breaking_bad = tv_shows.TV('Breaking Bad',
                           'A chemistry teacher and his desperation to earn money because of his illness'
                           ,
